namespace Xpense.Adapters.Postgres.Exceptions
{
    public class PriorityCreationFailedException(string name, Exception? innerException = null)
        : XpenseException($"Failure attempt to create priority {name}", innerException);

    public class PriorityDeletionFailedException(int id, Exception? innerException = null)
        : XpenseException($"Failure attempt to remove priority with id:[{id}]", innerException);
    ...
    public class PriorityUpdateFailedException(int id, Exception? innerException = null)
        : XpenseException($"Failure attempt to update priority with id:[{id}]", innerException);
}
